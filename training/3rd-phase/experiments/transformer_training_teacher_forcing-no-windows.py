...
def train_and_evaluate_model():
    ...
    # Prepare datasets
    inputs_scaled = input_scaler.transform(train_df[input_columns])
    outputs_scaled = output_scaler.transform(train_df[output_columns])
    train_inputs_tensor = torch.tensor(inputs_scaled, dtype=torch.float32)
    train_outputs_tensor = torch.tensor(outputs_scaled, dtype=torch.float32)
    ...
    with torch.no_grad():
        for input_window, actual_target_data_window in test_loader:
            ...
            decoder_input = torch.zeros((input_window.shape[0], 1, OUTPUT_SIZE), device=device)
            outputs = []

            for _ in range(WINDOW_SIZE):
                print("Input window shape:", input_window.shape)
                print("Decoder input shape:", decoder_input.shape)

                # Forward pass through the model
                output = model(input_window, decoder_input)

                # Use the latest output as the next input for the decoder
            ...
            # Concatenate all the outputs to form the complete sequence
            full_output = torch.cat(outputs, dim=1)
            predictions.extend(full_output.cpu().numpy())
