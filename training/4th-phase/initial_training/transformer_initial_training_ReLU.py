...
BATCH_SIZE = 32
hidden_size = 20  # 8 hidden layers produce NaN loss, 5 Produces good results, 10 produces very good results
INPUT_SIZE = 5
OUTPUT_SIZE = 6
...
class TransformerModelWithTwoAuxEncoders(nn.Module):
    ...
    def forward(self, jobs, nodes, links, tgt):
        # Project to hidden size and apply ReLU
        jobs = self.relu(self.jobs_src_input_projection(jobs))
        nodes = self.relu(self.nodes_aux_input_projection(nodes))
        links = self.relu(self.links_aux_input_projection(links))
        tgt = self.relu(self.tgt_input_projection(tgt))
...
def train_and_evaluate_model():
    ...
    plotting.denorm_and_plot_predicted_actual(output_columns_jobs, output_jobs_scaler, predictions_array, actual_values_array, model_name, purpose="training-ReLU")
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), '../generated-models/transformer-relu_weights.pth')
    torch.save(model, '../generated-models/transformer-relu.pth')
